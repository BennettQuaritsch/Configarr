...
> List[SeriesResource] api_v3_series_lookup_get(term=term)
...
with sonarr_api.ApiClient(configuration) as api_client:
    ...
    api_instance = sonarr_api.SeriesLookupApi(api_client)
    term = 'term_example' # str |  (optional)

    try:
        api_response = api_instance.api_v3_series_lookup_get(term=term)
        print("The response of SeriesLookupApi->api_v3_series_lookup_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SeriesLookupApi->api_v3_series_lookup_get: %s\n" % e)
```
